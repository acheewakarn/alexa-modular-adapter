...
std::shared_ptr<ToggleControllerHandler> ToggleControllerHandler::create(std::string endpointId) {
    auto toggleControllerHandler = std::shared_ptr<ToggleControllerHandler>(new ToggleControllerHandler(endpointId));
    return toggleControllerHandler;
}

ToggleControllerHandler::ToggleControllerHandler(std::string endpointId) : m_endpointId(endpointId), m_currentToggleState{false} {
}
...

    {
        ...
        if (m_currentToggleState != state) {
            ...
            if (state) {
                ...
                //TODO: Insert Wire Pi code to toggle on device

            } else { 

                ConsolePrinter::prettyPrint("TOGGLE CONTROLLER TURN-OFF");
                
                //TODO: Insert Wire Pi code to toggle off device
